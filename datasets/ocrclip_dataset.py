...
class OCRCLIPDataset(IcdarDataset): #验证时候的prompt
    CLASSES = ('text',)
    ...
    def __init__(self,
        ...
        # self.alpha_prefix = alpha_prefix #改AlphaTCM10
    ...
    def _log_filter_error_index(self, err_index, replace_idx):
        ...
        try:
            ...
            # alpha_prefix = self.alpha_prefix #改AlphaTCM11
            ...
            # # ___________#改AlphaTCM12
            # print_log(f'Warning: skip broken error file {data_info} '
            #           f'with alpha_prefix {alpha_prefix}, '
            #           f'replace with index {replace_idx}, info: {self.data_infos[replace_idx]}')
    ...
    def prepare_train_img(self, idx):
        ...
        try:
            ...
            img_pipeline = self.pipeline(results)
            # # __________________________#改AlphaTCM13
            # alpha_img_info = img_info
            # alpha_ann_info = ann_info
            # alpha_img_info['file_name'] = alpha_img_info['alphafilename']
            # alpha_img_info['filename'] = alpha_img_info['alphafilename']
            # s = ann_info['seg_map']
            # name = s.split('/')[-1]
            # alpha_ann_info['seg_map'] = 'alpha/' + 'mask_' + name
            # alpha_results = dict(img_info=alpha_img_info, ann_info=ann_info)
            # self.pre_pipeline(alpha_results)
            # alpha_pipeline = self.pipeline(alpha_results)
            # img_pipeline['alpha'] = alpha_pipeline['img']
            ...
            img_prefix = self.img_prefix
            # alpha_prefix = self.alpha_prefix #改AlphaTCM14
            print_log(f'Warning: skip broken error file {data_info} '
                      f'with img_prefix {img_prefix}, ')
            # #改AlphaTCM15
            # print_log(f'Warning: skip broken error file {data_info} '
    ...
    def load_annotations(self, ann_file):
        ...
        for i in self.img_ids:
            ...
            # # _______________________________________________#改AlphaTCM16
            # img_filename = info['filename']
            # name = img_filename.split('/')[-1]
            # alphafilename = 'alpha/' + 'mask_' + name
            # info['alphafilename'] = alphafilename
...
class VisOCRCLIPDataset(OCRCLIPDataset):
    ...
    # PROMPT_CLASSES = ('the set of many arbitrary-shape text instances.',)
    PROMPT_CLASSES = ('the pixels of many arbitrary-shape text instances.',)
    # INSTANCE_CLASSES = ('first text', 'second text')    # PROMPT_CLASSES = ('a set of texts, including horizontal, multi-oriented and curved text.',)
    INSTANCE_CLASSES = [num2words(x, ordinal=True) + ' text' for x in range(1, 101)]
